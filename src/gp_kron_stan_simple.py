...
def kern_func_sqr_exp(lengthscale, x1, x2):
    """
    If x1 and x2 are 1D call function as kern_func_sqr_exp(lengthscale, x1[:, None], x2[None, :]).
    """
    return np.exp(-0.5 * ((x1 - x2) / lengthscale) ** 2)


def kern_func_periodic_sqr_exp(lengthscale, w1, w2):
    """
    If x1 and x2 are 1D call function as kern_func_sqr_exp(lengthscale, x1[:, None], x2[None, :]).
    """
    return np.exp(-2 * np.sin(0.5 * np.abs(w1 - w2)) ** 2 / lengthscale ** 2)


# Callable[[1D, 1D], 2D]
KernelFunc = Callable[[np.ndarray, np.ndarray], np.ndarray]


class GPModel(ABC):

    def __init__(self, df, fe_mu_formula=None, re_mu_formulas=None, fe_noise_formula=None, re_noise_formulas=None,
                 priors=None, kern_func_freq: KernelFunc = None, kern_func_phase: KernelFunc = None):
        ...
        # kernel functions, without the variance coefficient
        self.kern_func_freq = kern_func_freq
        self.kern_func_phase = kern_func_phase

        # set default priors
        varnames = ['tau_gamma', 'tau_beta', 'sigma_noise']
    ...
    def get_params_fe(cls) -> List[str]:
        return ['beta', 'gamma', 'offset_eta', 'sigma_noise', 'tau_beta', 'tau_gamma', 'noise']
...
class GPFreqModel(GPModel):
    ...
    def __init__(self, df, fe_mu_formula=None, re_mu_formulas=None, fe_noise_formula=None, re_noise_formulas=None,
                 priors=None, kern_func_freq: KernelFunc = None):
    ...
    def get_template(cls):
        ...
        codepath = Path(inspect.getfile(this_module)).parent / 'gp1d_simple_template.stan'
    ...
    def set_data(self, y, nugget_size=1e-6, **kwargs):
        freqs = kwargs['freqs']
        self.freqs = freqs
        ...
        # Set kernel cholesky.
        kern = self.kern_func_freq(freqs[:, None], freqs[None, :])
        kern_chol = np.linalg.cholesky(kern + nugget_size * np.identity(kern.shape[0]))
        self.stan_input_data['kern_chol'] = kern_chol

    @classmethod
    def _interp(cls, kern_x_chol, kern_x_star, y):
        ...
        y_star = np.empty((nsamples, kern_x_star.shape[0]))
        for i in range(nsamples):
            alpha = scipy.linalg.cho_solve((kern_x_chol, True), y[i])
            y_star[i, ...] = (kern_x_star @ alpha).flatten()

        return y_star

    def interp_samples(self, samples, x, x_star, nugget_size=1e-6):

        nx_star = len(x_star)
        samples_star = dict()

        nugget_x = nugget_size * np.identity(len(x))
        kern_x = self.kern_func_freq(x[:, None], x[None, :])
        kern_x_star = self.kern_func_freq(x_star[:, None], x[None, :])
        kern_x_chol = np.linalg.cholesky(kern_x + nugget_x)

        # interpolate fixed-effects
        for param in ['beta', 'gamma']:

            v = samples[param]
            nsamples, ncols, _ = v.shape

            # interpolate param
            v_star = np.empty((nsamples, ncols, nx_star))
            for j in range(ncols):
                v_star[:, j, :] = self._interp(kern_x_chol, kern_x_star, v[:, j, ...])
...
class GPFreqPhaseModel(GPModel):
    ...
    def __init__(self, df, fe_mu_formula=None, re_mu_formulas=None, fe_noise_formula=None, re_noise_formulas=None,
                 priors=None, sep=None, kern_func_freq: KernelFunc = None, kern_func_phase: KernelFunc = None):
        super().__init__(df, fe_mu_formula, re_mu_formulas, fe_noise_formula, re_noise_formulas, priors,
                         kern_func_freq, kern_func_phase)
        self.sep = sep
    ...
    def get_template(cls):
        this_module = sys.modules[cls.__module__]
        codepath = Path(inspect.getfile(this_module)).parent / 'gp2d_simple_template.stan'
        with codepath.open('r') as f:
            return f.read().splitlines()
    ...
    def set_data(self, y, nugget_size=1e-6, **kwargs):
        ...
        kern_freq = self.kern_func_freq(freqs[:, None], freqs[None, :])
        kern_phase = self.kern_func_phase(phases[:, None], phases[None, :])
        kern_freq_chol = np.linalg.cholesky(kern_freq + nugget_size * np.identity(kern_freq.shape[0]))
        kern_phase_chol = np.linalg.cholesky(kern_phase + nugget_size * np.identity(kern_phase.shape[0]))
        self.stan_input_data['kern_chol_f'] = kern_freq_chol
        self.stan_input_data['kern_chol_h'] = kern_phase_chol
    ...
    @classmethod
    def _interp(cls, kern_x, kern_x_star, kern_w, kern_w_star, y, nugget_size):

        nsamples, n = y.shape

        nugget_x = nugget_size + np.identity(kern_x.shape[0])
        nugget_w = nugget_size + np.identity(kern_w.shape[0])
        inv_kern_x = np.linalg.inv(kern_x + nugget_x)
        inv_kern_w = np.linalg.inv(kern_w + nugget_w)

        y_star = np.empty((nsamples, kern_x_star.shape[0] * kern_w_star.shape[0]))
        for i in range(nsamples):
    ...
    def interp_samples(self, samples, x, w, x_star, w_star, nugget_size=1e-6):
        ...
        kern_x = self.kern_func_freq(x[:, None], x[None, :])
        kern_x_star = self.kern_func_freq(x_star[:, None], x[None, :])
        kern_w = self.kern_func_freq(w[:, None], w[None, :])
        kern_w_star = self.kern_func_freq(w_star[:, None], w[None, :])
        ...
        for param in ['beta', 'gamma']:
            ...
            for j in range(ncols):
                v_star[:, j, :] = self._interp(kern_x, kern_x_star, kern_w, kern_w_star, v[:, j, ...], nugget_size)
